                                    ...
                                    <div className="flex-1 mb-8 space-y-4">
                                        {(cochera.vehicleDetails || cochera.vehiculos || []).length > 0 ? (
                                            (cochera.vehicleDetails || cochera.vehiculos).map((v: any, idx: number) => {
                                                // Ultra-safe extraction to prevent Object-as-child recursion
                                                const safePlate = (val: any) => {
                                                    if (typeof val === 'string') return val;
                                                    if (val && typeof val === 'object') return val.plate || 'S/D';
                                                    return '---';
                                                };

                                                const plateText = safePlate(v);
                                                const brandText = typeof v === 'object' ? v.brand : null;
                                                const modelText = typeof v === 'object' ? v.model : null;

                                                return (
                                                    <div key={idx} className="flex items-center gap-4">
                                                        <div className="relative group/plate">
                                                            <div className="px-4 py-1.5 bg-gradient-to-b from-gray-800 to-gray-950 border border-gray-700 border-b-2 border-b-black shadow-inner rounded flex items-center justify-center min-w-[110px]">
                                                                <span className="font-mono text-emerald-400 font-bold tracking-widest text-sm uppercase drop-shadow-sm">
                                                                    {plateText}
                                                                </span>
                                                            </div>
                                                            <div className="absolute top-1.5 left-1.5 w-1 h-1 rounded-full bg-gray-950 shadow-[0_1px_0_rgba(255,255,255,0.1)]"></div>
                                                            <div className="absolute top-1.5 right-1.5 w-1 h-1 rounded-full bg-gray-950 shadow-[0_1px_0_rgba(255,255,255,0.1)]"></div>
                                                            <div className="absolute bottom-1.5 left-1.5 w-1 h-1 rounded-full bg-gray-950 shadow-[0_1px_0_rgba(255,255,255,0.1)]"></div>
                                                            <div className="absolute bottom-1.5 right-1.5 w-1 h-1 rounded-full bg-gray-950 shadow-[0_1px_0_rgba(255,255,255,0.1)]"></div>
                                                        </div>
                                                        {brandText && <span className="text-xs text-gray-500 font-medium uppercase tracking-wider">{brandText} {modelText}</span>}
                                                    </div>
                                                )
                                            })
                                        ) : (
